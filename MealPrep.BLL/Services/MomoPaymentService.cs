...
public class MomoPaymentService : IMomoService
{
    ...
    public async Task<string> CreatePaymentRequestAsync(Payment payment, string returnUrl, string ipnUrl)
    {
        ...
        // Generate signature
        var rawSignature = $"accessKey={accessKey}&amount={requestData.amount}&extraData={requestData.extraData}&ipnUrl={ipnUrl}&orderId={orderId}&orderInfo={requestData.orderInfo}&partnerCode={partnerCode}&redirectUrl={returnUrl}&requestId={requestId}&requestType={requestData.requestType}";
        ...
        {
            throw new InvalidOperationException($"MoMo API error: {response.StatusCode} - {responseContent}");
        }

        // Parse response
        var responseData = JsonSerializer.Deserialize<JsonElement>(responseContent);
        
        if (responseData.TryGetProperty("resultCode", out var resultCode) && 
