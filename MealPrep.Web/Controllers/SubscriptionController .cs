    ...
    public class SubscriptionController : Controller
    {
        ...
        public async Task<IActionResult> Checkout(CheckoutVm vm)
        {
            ...
            try
            {
                ...
                {
                    TempData["ErrorMessage"] = "Hi?n t?i ch? h? tr? dang ký gói 2 b?a/ngày.";
                    return RedirectToAction(nameof(Index));
                }
            ...
            catch (Exception ex)
            {
                ...
                TempData["ErrorMessage"] = $"Có l?i x?y ra khi t?o dang ký: {ex.Message}";
                return RedirectToAction(nameof(Index));
            }
        }

        /// <summary>
        /// Thanh toán l?i cho m?t subscription dang ? tr?ng thái Ch? thanh toán.
        /// </summary>
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> RetryPayment(int subscriptionId)
        {
            ...
            try
            {
                // T?o ho?c l?y payment Pending hi?n có cho subscription
            ...
            catch (Exception ex)
            {
                ...
                TempData["ErrorMessage"] = $"Có l?i x?y ra khi t?o l?i thanh toán: {ex.Message}";
                return RedirectToAction("Details", "UserSubscriptions", new { id = subscriptionId });
            }
        }
            ...
            [FromQuery] string? partnerCode)
        {
            ...
            if (isSuccess && !string.IsNullOrEmpty(orderId))
            {
                try
                {
                    ...
                    {
                        TempData["SuccessMessage"] = "Thanh toán thành công! Gói dang ký c?a b?n dã du?c kích ho?t.";
                    ...
                    {
                        TempData["ErrorMessage"] = "Thanh toán thành công nhung không tìm th?y thông tin gói dang ký. Vui lòng liên h? h? tr?.";
                        return RedirectToAction(nameof(Index));
                    }
                }
                catch (InvalidOperationException ex)
                {
                    if (ex.Message.Contains("already processed"))
                    {
                        ...
                        try
                        {
                            ...
                            if (subscription != null)
                            {
                                TempData["SuccessMessage"] = "Thanh toán dã du?c x? lý tru?c dó. Gói dang ký c?a b?n dã du?c kích ho?t.";
                        ...
                        TempData["SuccessMessage"] = "Thanh toán dã du?c x? lý tru?c dó.";
                    ...
                    else if (ex.Message.Contains("Payment not found"))
                    {
                        ...
                        TempData["ErrorMessage"] = "Không tìm th?y thông tin thanh toán. Vui lòng liên h? h? tr?.";
                        return RedirectToAction(nameof(Index));
                    }
                    else if (ex.Message.Contains("trùng") || ex.Message.Contains("overlap") || ex.Message.Contains("Không th? kích ho?t"))
                    {
                    ...
                    TempData["ErrorMessage"] = $"Có l?i x?y ra khi x? lý thanh toán: {ex.Message}";
                    return RedirectToAction(nameof(Index));
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to confirm payment");
                    TempData["ErrorMessage"] = "Có l?i x?y ra khi x? lý thanh toán. Vui lòng liên h? h? tr?.";
                }
            }
            else
            {
