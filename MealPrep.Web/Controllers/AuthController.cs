    ...
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterVm vm)
        {
            ...
            try
            {
                ...
                // Sau khi dang ký thành công, chuy?n d?n trang hoàn t?t thông tin cá nhân
        ...
        /// <summary>
        /// Trang hoàn t?t thông tin cá nhân (tu?i, gi?i tính, s? di?n tho?i) - GET
        /// </summary>
        [Authorize]
        [HttpGet]
        ...
        /// <summary>
        /// X? lý submit form hoàn t?t thông tin cá nhân - POST
        /// </summary>
        [Authorize]
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CompleteProfile(CompleteProfileVm vm)
        {
            ...
            try
            {
                ...
                TempData["SuccessMessage"] = "Hoàn t?t thông tin cá nhân thành công!";
                
                // Sau khi hoàn t?t thông tin cá nhân, chuy?n d?n trang thi?t l?p nutrition profile
                return RedirectToAction(nameof(SetupNutritionProfile));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error completing profile");
                ModelState.AddModelError("", "Ðã x?y ra l?i khi c?p nh?t h? so. Vui lòng th? l?i.");
                return View("Index", vm);
            }
        }
        ...
        /// <summary>
        /// X? lý submit form thi?t l?p h? so dinh du?ng - POST
        /// </summary>
        [Authorize]
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SetupNutritionProfile(UpdateNutritionProfileVm vm)
        {
            ...
            try
            {
                ...
                TempData["SuccessMessage"] = "Thi?t l?p h? so dinh du?ng thành công! Chào m?ng b?n d?n v?i MealPrep.";
                
                // Sau khi hoàn t?t nutrition profile, chuy?n d?n Dashboard
                return RedirectToAction("Index", "Dashboard");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error setting up nutrition profile for user {UserId}", User.FindFirstValue(ClaimTypes.NameIdentifier));
                ModelState.AddModelError("", "Ðã x?y ra l?i khi thi?t l?p h? so dinh du?ng. Vui lòng th? l?i.");
                return View(vm);
            }
        }
            ...
            int mealsPerDay, int? caloriesInDay, string? notes, List<string>? allergies)
        {
            ...
            try
            {
                ...
                TempData["SuccessMessage"] = "Ðã c?p nh?t h? so dinh du?ng thành công!";
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to update nutrition profile");
                TempData["ErrorMessage"] = "Có l?i x?y ra khi c?p nh?t h? so. Vui lòng th? l?i.";
            }
        ...
        public async Task<IActionResult> AddDislikedMeal(int mealId)
        {
            ...
            try
            {
                ...
                TempData["SuccessMessage"] = "Ðã thêm món vào danh sách không thích.";
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> RemoveDislikedMeal(int dislikedMealId)
        {
            ...
            try
            {
                ...
                if (dislikedMeal != null)
                {
                    ...
                    TempData["SuccessMessage"] = "Ðã xóa món kh?i danh sách không thích.";
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ToggleDislikedMeal(int mealId)
        {
            ...
            try
            {
                ...
                if (isDisliked)
                {
                    ...
                    TempData["SuccessMessage"] = "Ðã b? không thích món này.";
                }
                else
                {
                    await _userService.AddDislikedMealAsync(userId, mealId);
